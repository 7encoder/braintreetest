...
class GFStripe extends GFPaymentAddOn {
	...
	/**
     * Adds support for payment status conditional logic for the refactored payment methods.
     *
     * @since 6.0
     *
     * @param array $form The current form.
    ...
    public function get_conditional_logic_payment_statuses( $form ) {
        ...
        if ( $this->is_refactored_payment_method( $form ) ) {
	        return array(
		        'Active'     => esc_html__( 'Active', 'gravityformsstripe' ),
		        'Paid'       => esc_html__( 'Paid', 'gravityformsstripe' ),
		        'Processing' => esc_html__( 'Processing', 'gravityformsstripe' ),
	        );
	    }

        // Payment methods that haven't been refactored yet do not support payment status conditional logic.
        return array();
    }
	/**
		...
		if ( ! $is_webhook_processed ) {
			...
			switch ( $type ) {
				...
				case 'invoice.payment_succeeded':
					$subscription_details = rgars( $event, 'data/object/parent/subscription_details' );
					if (!$subscription_details) {
						return new WP_Error('invalid_request', sprintf(__('Subscription line item not found in request', 'gravityformsstripe')));
					}

					$action['subscription_id'] = rgar($subscription_details, 'subscription');
					$entry_id = $this->get_entry_by_transaction_id($action['subscription_id']);
					if (!$entry_id) {
						return $this->get_entry_not_found_wp_error('subscription', $action, $event);
					}
